...
def sized_mf_sampling(idx, vals, p, nodes, nodes_to_neighbors, hyperedges):
...
def negative_sample(nodes_to_neighbors, size_dist, num_negative, hyperedges, method, corrupt_num=1, half=False, rw_path=None):
    ...
    if method in ['SNS', 'UNS']:
        for i in range(num_negative):
            ...
            sampled_edge = sized_random_sampling(size_dist, nodes, nodes_to_neighbors, hyperedges)
        ...
        #     if i % 1000 == 0:
        #         print(f"{method} {i}/{num_negative}")
        #     sampled_edge = sized_mf_sampling(vals,p, nodes, nodes_to_neighbors, hyperedges)
        #     neg_samples.append(sampled_edge)
        import multiprocessing as mp
        cpu_count =  max(mp.cpu_count() - 4, 1)
        print(f"cpu_count = {cpu_count};  num_negative = {num_negative}")
        pool = mp.Pool(cpu_count)
        results = [pool.apply_async(sized_mf_sampling, args=(idx, vals, p, nodes, nodes_to_neighbors, hyperedges)) for idx in range(num_negative)]
        ...
        for i in range(num_negative):
            if i % 100 == 0:
                print(f"{method} {i}/{num_negative}")
            sampled_edge = clique_negative_sampling(hyperedges, nodes_to_neighbors, num_negative, list_hyperedges, node_set)
            neg_samples.append(sampled_edge)
...
def generate_negative_samples_for_hyperedges(hyperedges, method, neg_samples_size, corrupt_num = 1, half=False, rw_path = None):
    ...
    neg_samples = negative_sample(nodes_to_neighbors, size_dist, total, hyperedges, method, corrupt_num = 1, half=False, rw_path = rw_path)
