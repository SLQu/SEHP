...
def clique_negative_sampling(
    ...
    while neg in hyperedges:
        ...
        for node in nodes_to_keep:
            probable_neighbors = probable_neighbors.intersection(nodes_to_neighbors[node])
...
def mfinder_sampling(k):
    ...
    while len(sampled_nodes) < k:
        ...
        neighbor_edges = [edge for edge in neighbor_edges if new_node not in edge]
...
# def sized_mf_sampling(idx, vals, p, nodes, hyperedges):
def sized_mf_sampling(vals, p, nodes, hyperedges):

    # if idx % 200 == 0:
    #     print(f"time hh:mm:ss = {time.strftime('%H:%M:%S')}")
    #     print(f"Mfinder Negative Sampling {idx}/{idx}")
    ...
    while frozenset(sampled_nodes) in hyperedges:
        sampled_nodes, sampled_edge = mfinder_sampling( sampled_size)
    hyperedges.remove(frozenset({'a', 'b'}))
    return sampled_nodes, sampled_edge

def sized_random_sampling(vals, p, nodes, hyperedges):
    # print('sized_random_sampling')
    # vals = [v for v, p in size_dist]
    # p = [p for v, p in size_dist]
...
def negative_sample( size_dist, num_negative, hyperedges, method, corrupt_num=1, half=False, rw_path=None):
    nodes = list(nodes_to_neighbors.keys())
    neg_samples = []
    print(f"current time hh:mm:ss = {time.strftime('%H:%M:%S')}")
    print(f"Negative Sampling Method : {method}")
    if method == 'UNS':
    ...
    if method in ['SNS', 'UNS']:
        for i in range(num_negative):
            if i % 100 == 0:
                print(f"{method} {i}/{num_negative}")
            sampled_edge = sized_random_sampling(vals, p, nodes, hyperedges)
        ...
        for i in range(num_negative):
            ...
            sampled_edge = sized_mf_sampling(vals,p, nodes, hyperedges)
            neg_samples.append(sampled_edge)

        # import multiprocessing as mp
        # cpu_count =  max(mp.cpu_count() - 4, 1)
        # print(f"cpu_count = {cpu_count};  num_negative = {num_negative}")
        # pool = mp.Pool(cpu_count)
        # results = [pool.apply_async(sized_mf_sampling, args=(idx, vals, p, nodes, hyperedges)) for idx in range(num_negative)]
        # neg_samples = [result.get() for result in results]
        # pool.close()
        # pool.join()
        ...
        for i in range(num_negative):
            if i % 100 == 0:
                print(f"{method} {i}/{num_negative}")
            sampled_edge = clique_negative_sampling(hyperedges, num_negative, list_hyperedges, node_set)
            neg_samples.append(sampled_edge)
...
#             if i % 1000 == 0:
#                 print(f"{method} {i}/{num_negative}")
#             sampled_edge = sized_random_sampling(size_dist, nodes, hyperedges)
#             neg_samples.append(sampled_edge)
#     elif method == 'MNS':
#         vals = [v for v, p in size_dist]
#         p = [p for v, p in size_dist]
...
#                 hyperedges, num_negative, list_hyperedges,
...
'''
有这样一个python的任务。给定一个hyperedges =  {frozenset({1736, 1734, 1735}), frozenset({1360, 1361, 1362}), frozenset({1153, 1154, 1155, 1741}), frozenset({1736, 5, 6, 2220, 2221, 238, 239}), frozenset({608, 609}), frozenset({2233, 1655}), frozenset({503, 1253, 1254, 1959, 1768, 1769, 2385, 983, 1489, ...}), frozenset({1256, 1004, 1005, 1255}), frozenset({1178, 1332, 1333, 1334, 1335, 1336, 1337, 1338, 1339, ...}), frozenset({152, 151}), frozenset({1152, 1153, 1154, 67, 68, 69, 70, 1155, 1147, ...}), frozenset({517, 518})}
其中frozenset({1736, 1734, 1735})表示一个hyperedge，里面有三个节点1736, 1734, 1735。现在我想得每个节点的邻居节点，比如1736的邻居节点是5, 6, 238, 239, 1734, 1735,2220, 2221。请给出时间复杂度尽可能小的代码。

'''

from collections import defaultdict

def build_neighbor_graph(hyperedges):
    print(f" begin to build_neighbor_graph, current is {time.strftime('%H:%M:%S')}")
    # 使用defaultdict自动初始化集合，省去了手动检查和初始化的步骤
    neighbors = defaultdict(set)

    # 遍历每个hyperedge
    print(f" total hyperedges is {len(hyperedges)}")
    for _i_, edge in enumerate(hyperedges):
        if _i_ % 500 == 0:
            print(f" current hyperedge is {_i_}/{len(hyperedges)}, and time is {time.strftime('%H:%M:%S')}")
        for node in edge:
            # 将hyperedge中除自身外的其他节点加入当前节点的邻居集合
            neighbors[node].update(edge - {node})
    print(f" end to build_neighbor_graph, current is {time.strftime('%H:%M:%S')}")
    return neighbors

def generate_negative_samples_for_hyperedges(hyperedges, method, neg_samples_size, corrupt_num = 1, half=False, rw_path = None):
    #print(hyperedges)

    global nodes_to_neighbors

    nodes_to_neighbors = defaultdict(set)
    if method in ['SNS']:
        _node_ids_set = set([node for hedge in hyperedges for node in hedge])
        for n_i in _node_ids_set:
            nodes_to_neighbors[n_i] = 0
    else:    
        # edges = {
        ...
        nodes_to_neighbors = build_neighbor_graph(hyperedges)

    size_dist = generate_hyperedge_size_dist(hyperedges)
    
    #print('Generating Negative Samples')
    total = math.ceil(neg_samples_size)
    neg_samples = negative_sample( size_dist, total, hyperedges, method, corrupt_num = 1, half=False, rw_path = rw_path)
