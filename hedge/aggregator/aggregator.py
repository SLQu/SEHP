...
from torch_geometric.nn.aggr.basic import MaxAggregation, MinAggregation
class MaxMinAggregation(torch.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.max_agg = MaxAggregation()
        self.min_agg = MinAggregation()
    
    def forward(self, embeddings, edge = None):
        max_val = self.max_agg(embeddings, edge)
        min_val = self.min_agg(embeddings, edge)
        return max_val - min_val
